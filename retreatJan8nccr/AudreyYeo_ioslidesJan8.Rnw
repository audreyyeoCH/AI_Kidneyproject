...
ggplot(df) +
  geom_histogram(alpha=0.6, binwidth = 20, aes(x = v_GFR0)) +
...
ggplot(df[4000:10000,]) +
  geom_path(aes(y = log(v_GFR0), x = as.factor(time), group = , colour = as.factor(id))) + theme_gray() +
...
ggplot(df[4000:10000,]) +
  geom_path(aes(y = log(v_PA), x = as.factor(time), group = , colour = as.factor(id))) + theme_gray() +
